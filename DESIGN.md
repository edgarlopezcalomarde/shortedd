...
**Character:** A workhorse humanist sans for anything meant to be read, paired with its own monospace sibling for anything meant to be *data* — the pairing shares one type family's design language, so the switch between prose and code never feels like a costume change.
